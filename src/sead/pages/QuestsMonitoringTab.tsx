...
import { CompletionStatusTab } from "./CompletionStatusTab";

export function QuestsMonitoringTab() {
  const [tab, setTab] = useState<"scores" | "rankings" | "completion">("scores");
  ...
  return (
    <div>
      <div className="flex gap-1 border-b border-[#e6ecf5] mb-5">
        ...
        {/* New quest monitoring tasks, Task 2 */}
        <button onClick={() => setTab("completion")} className={`flex items-center gap-1.5 px-3 py-2 text-[12.5px] font-bold border-b-2 ${tab === "completion" ? "border-[#0088cc] text-[#062444]" : "border-transparent text-slate-400 hover:text-[#062444]"}`}>
          <ListChecks size={14} /> Completion Status
        </button>
      </div>
      {tab === "scores" ? <ScoresTab /> : tab === "rankings" ? <RankingsTab /> : <CompletionStatusTab />}
    </div>
  );
}
