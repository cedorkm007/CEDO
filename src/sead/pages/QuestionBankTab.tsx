...
import {
  fetchSubjects, createSubject, renameSubject, deleteSubject,
...
function SubjectColumn({ subjects, selected, onSelect, onCreate, onRename, onDelete }: {
  ...
  return (
    <ListColumn
      ...
      onSelect={id => { const s = subjects.find(x => x.id === id); if (s) onSelect(s); }}
      onCreate={onCreate}
      onRename={onRename}
      onDelete={onDelete}
      createPlaceholder="New subject (e.g. Mathematics)"
    />
  );
}
