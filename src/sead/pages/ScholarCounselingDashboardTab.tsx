...
import { Users, Home, AlertTriangle, Clock, CalendarDays, ChevronRight } from "lucide-react";
import { fetchDashboardSummary, fetchReferralQueue, type DashboardSummary, type QueuedReferral } from "../referralApi";
...
export function ScholarCounselingDashboardTab() {
  ...
  async function load() {
    setLoading(true);
    const [s, q] = await Promise.all([fetchDashboardSummary(), fetchReferralQueue()]);
    setSummary(s);
    setQueue(q);
    setLoading(false);
  }
