    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }}
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-8">
            ...
            {form.objectives.map((row, i) => (
              <div key={i} className="grid grid-cols-2 border-t border-gray-100 group">
                ...
                <div className="p-2 flex gap-1">
                  ...
                  {form.objectives.length > 1 && <button onClick={() => delObj(i)} className="text-red-300 hover:text-red-500 shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>}
                </div>
              </div>
            ))}
          </div>
          <button onClick={addObj} className="flex items-center gap-2 text-xs text-[#062444] font-semibold hover:underline mb-2"><Plus className="w-4 h-4" />Add Row</button>
            ...
            {form.workPlan.map((row, i) => (
              <div key={i} className="grid grid-cols-2 border-t border-gray-100 group">
                ...
                <div className="p-2 flex gap-1">
                  ...
                  {form.workPlan.length > 1 && <button onClick={() => delWP(i)} className="text-red-300 hover:text-red-500 shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>}
                </div>
              </div>
            ))}
          </div>
          <button onClick={addWP} className="flex items-center gap-2 text-xs text-[#062444] font-semibold hover:underline mb-2"><Plus className="w-4 h-4" />Add Row</button>
              ...
              <div key={i} className="grid grid-cols-4 border-t border-gray-100 group min-w-[480px]">
                {(["time", "segment", "deliverables", "personInCharge"] as const).map((col, ci) => (
                  <div key={col} className={`p-2 ${ci < 3 ? "border-r border-gray-100" : "flex gap-1"}`}>
                    ...
                    {ci === 3 && form.programFlow.length > 1 && <button onClick={() => delPF(i)} className="text-red-300 hover:text-red-500 shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>}
                  </div>
                ))}
              </div>
            ))}
          </div>
          <button onClick={addPF} className="flex items-center gap-2 text-xs text-[#062444] font-semibold hover:underline mb-2"><Plus className="w-4 h-4" />Add Row</button>
            ...
            {form.budgetItems.map((row, i) => (
              <div key={i} className="grid grid-cols-5 border-t border-gray-100 group min-w-[520px]">
                ...
                <div className="p-1.5 flex gap-1">
                  ...
                  {form.budgetItems.length > 1 && <button onClick={() => delBudget(i)} className="text-red-300 hover:text-red-500 shrink-0"><Trash2 className="w-3.5 h-3.5" /></button>}
                </div>
              </div>
            ))}
          ...
          <button onClick={addBudget} className="flex items-center gap-2 text-xs text-[#062444] font-semibold hover:underline mb-2"><Plus className="w-4 h-4" />Add Row</button>
