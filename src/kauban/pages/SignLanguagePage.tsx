      ...
      <div className="mx-auto max-w-3xl">
        ...
        {selected && (
          <div className="mb-6 rounded-2xl bg-white p-4 shadow-lg">
            <KaubanVideo
              ...
              autoPlay
            />
            <p className="mt-3 text-center text-lg font-bold text-[#1E1B3A]">{selected.label}</p>
          </div>
        )}

        {loading && <p className="py-8 text-center text-sm text-slate-400">Loading…</p>}
        {!loading && words.length === 0 && (
          <p className="py-8 text-center text-sm text-slate-400">No sign words have been added yet.</p>
        )}
        ...
        <div className="space-y-6">
          {categories.map(category => {
            ...
            return (
              <div key={category.id}>
                <h2 className="mb-2 text-sm font-bold text-[#1E1B3A]">{category.label}</h2>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                  {categoryWords.map(word => (
                    <button
                      key={word.id}
                      onClick={() => setSelectedId(word.id)}
                      className={`flex flex-col items-center gap-2 rounded-xl border-2 bg-white p-4 text-center shadow-sm transition hover:shadow-md focus:outline-none focus-visible:ring-4 focus-visible:ring-[#4F46E5]/30 ${selectedId === word.id ? "border-[#4F46E5]" : "border-transparent"}`}
                    >
                      <span className="flex h-11 w-11 items-center justify-center rounded-full bg-[#4F46E5]/10 text-[#4F46E5]">
                        {selectedId === word.id ? <Play size={18} /> : <Hand size={18} />}
                      </span>
                      <span className="text-sm font-semibold text-[#1E1B3A]">{word.label}</span>
                    </button>
                  ))}
                </div>
              </div>
