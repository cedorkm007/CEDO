 ...
 * (see offlineVideoDownload.ts). Triggered from KaubanTopNav's dropdown.
 */
export function OfflineDownloadModal({ onClose }: { onClose: () => void }) {
  ...
  async function handleStart() {
    ...
    const result = await downloadAllVideosForOffline(setProgress);
    setProgress(result);
    setStatus("done");
  }
        ...
        {status === "idle" && (
          <>
            <p className="mb-5 text-sm text-[#718096]">
              Downloads every sign-language video now, so they all work without an internet connection later. This can use a fair amount of mobile data — Wi-Fi is recommended.
            </p>
            <button
              onClick={handleStart}
